...
def NumberOfBitsJpeg(JpegInfo: tuple, N: int):
    """Calculates the total number of bits, including header file for DCT compression scheme."""
...
def JpegOptimiseConstStep(X: np.ndarray, InitStep: float, Increment: float, MaxError: float, Target: float = 40960, N: int = 8, M: int = 8,
        opthuff: bool = False, dcbits: int = 8, log: bool = True
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:

        """Function that finds step size that is closest to maximum number of bits, using the DCT compression scheme"""

        print("Starting DCT Optimisation")
        qstep = InitStep
        qprev = 0

        JpegInfo = jpegenc(X, qstep, N = N, M = M, dcbits = dcbits, log = False, opthuff =True)

        bits = NumberOfBitsJpeg(JpegInfo, N)
        err = Target - bits

        while err > MaxError:
            JpegInfo1 = jpegenc(X, qstep + Increment, N=N, M=M, dcbits= dcbits,  opthuff =True, log = False)
            JpegInfo2 = jpegenc(X, qstep - Increment, N=N, M=M, dcbits= dcbits,  opthuff =True, log = False)
        ...
        if bits > Target:
            JpegInfo = jpegenc(X, qprev, N = N, M = M, dcbits = dcbits, log = True, opthuff = True)
            bits = NumberOfBitsJpeg(JpegInfo, N)
            err = Target - bits

        # JpegInfo is what jpegenc returns, i.e. the vlc and information about the huffman encoding
        return JpegInfo, bits, err, qstep

def dwtenc(X:np.array, levels, dwtqstep, BlockSize, log: bool = True, opthuff: bool = False, dcbits: int = 8):
    """ Encoding function for the dwt"""
    Y = nlevdwt(X, levels)
...
def dwtdec(vlc: np.ndarray, dwtqstep: float, levels: int,  BlockSize: int = 8,
    ...
    Parameters:
        ...
        vlc: variable length output code from dwtenc
        qstep: quantisation step to use in decoding
...
def NumberOfBitsDwt(dwtInfo: tuple, N: int):
    """Calculates number of bits, including the header, for an image using the DWT."""
...
def dwtOptimiseConstStep(X: np.ndarray, InitStep: float, Increment: float, MaxError: float, levels: int, Target: float = 40960, BlockSize: int = 8,
        opthuff: bool = False, dcbits: int = 8
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:

        """Calculates the step size that results in the compressed image having close to the maximum allowable number of bits, using DWT with constant
        step size between layers."""

        print("DWT constant step optimisation started")

        # Calcs initial quantisation matrix to based on impulse response for the number of levels in the DWT
        qstep = InitStep
        prevstep = 0
        dwtstep = dwtQuantMatrix(qstep, levels, EqualStep = True)
        dwtInfo = dwtenc(X, levels, dwtstep, BlockSize, dcbits = dcbits, opthuff = True, log = False)
        bits = NumberOfBitsDwt(dwtInfo, levels)
        err = Target - bits

        while err > MaxError:
            dwtstep1 = dwtQuantMatrix(qstep + Increment, levels, EqualStep= True)
            dwtInfo1 = dwtenc(X, levels, dwtstep1, BlockSize,  log = False, opthuff =True, dcbits =dcbits)

            dwtstep2 = dwtQuantMatrix(qstep - Increment, levels, EqualStep= True)
            dwtInfo2 = dwtenc(X, levels, dwtstep2, BlockSize,  log = False, opthuff =True, dcbits =dcbits)
        ...
        if bits > Target:
            dwtstep = dwtQuantMatrix(prevstep, levels, EqualStep = True)
            dwtInfo = dwtenc(X, levels, dwtstep, BlockSize, dcbits = dcbits, opthuff = True, log = False)
            bits = NumberOfBitsDwt(dwtInfo, levels)
            err = Target - bits

        # DwtInfo is what dwtenc returns, i.e. the vlc and information about the huffman encoding
...
def dwtOptimiseEqualMse(X: np.ndarray, InitStep: float, Increment: float, MaxError: float, levels: int, Target: float = 40960, BlockSize: int = 8,
        ...
        print("DWT MSE optimisation started")
        qstep = InitStep
        prevstep = 0
        ...
        dwtInfo = dwtenc(X, levels, dwtstep, BlockSize,  log = False, opthuff =True, dcbits =dcbits)
        bits = NumberOfBitsDwt(dwtInfo, levels)
        err = Target - bits
        ...
        # If while loop exited with the number of bits greater than the max amount, revert to previous step.
        if bits > Target:
            dwtstep = dwtQuantMatrix(prevstep, levels, energy_out, EqualStep = False, EqualMse = True)
            dwtInfo = dwtenc(X, levels, dwtstep, BlockSize, dcbits = dcbits, opthuff = True, log = False)
            bits = NumberOfBitsDwt(dwtInfo, levels)
            err = Target - bits
