...
def dwtImpulseResponse(levels):
    """Returns the energies when an impulse is applied to an image before reconstructing with the inverse DWT"""
...
def dwtQuantMatrix(qstep, levels, energies: np.array = np.zeros((256, 256)), EqualStep: bool = True, EqualMse: bool = False):
    """Calculates the quantisation matrix for a DWT, can be set to be either for equal step DWT or equal MSE DWT."""
    ...
    if EqualStep == 1:
        ...
        "Error: one of constant step and equal mse must be true"